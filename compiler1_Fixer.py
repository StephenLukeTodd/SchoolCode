#Logan Simpson
#January 16, 2017
#CSC330 - Winter 2017 Dr. Gourd
#Compiler 1 project

#This is a modified python script that will interpret a language and decide whether
#it can be compiled or not. Teacher provided 2 success test cases and 4 failure test
#case. This script was successful on the 2 success cases and returned an error on the 
#4 failure cases.

import sys

norw = 22      #number of reserved words increased from 11 to 22
...
def error(num):
    ...
    #Added some new error messages at the end
    print
    if num == 1: 
...
def block(tableIndex):
    ...
    tx[0] = tableIndex
    global sym, id;
    #Added while loop here to handle restricted globals
    while (sym == "CONST" or sym == "VAR"):
        if sym == "CONST":
...
def statement(tx):
    ...
    #Modified this block to add the else feature
    elif sym == "IF":
        getsym()
        condition(tx)
        if sym != "THEN":
    ...
    #New Block Added
        ...
        if i==0:
            error(11)
        #Checks to make sure ident is a variable
        elif table[i].kind != "variable":
            error(12)
        getsym()
        if sym != 'becomes':
    ...
    #New Block
        ...
        while True:
            ...
            if sym == 'CEND':
                break
            elif sym != 'number' and sym != 'ident':
                error(30)
            else:
                if sym == 'ident':
                    i = position(tx, id) 
                    if i==0: error(11)
                    #Checks to see if ident is a const
...
#Added several new reserved words to the main program. New ones have asterisk
rword.append('BEGIN')
rword.append('CALL')
rword.append('CASE') #*
rword.append('CEND') #*
