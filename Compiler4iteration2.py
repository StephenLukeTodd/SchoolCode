...
Where do I pass in the parameters for an array of booleans? 
'''
import sys, string
...
def block(tableIndex, level):
    ...
    cx1 = codeIndx
    gen("JMP", 0 , 0)
    #Needed to add in REF to be able to interpret functions properly. 
...
def factor(tx, level):
    ...
    if sym == "ident":
        ...
        #Instead of calling expression, we can call GeneralExpression
        print sym 
        ###Error to fix
        if sym != "rparen":
            error(22)
