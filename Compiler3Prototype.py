...
Date: 2 - 6 - 2017
Project: Compiler Assignment 3. The task is to extend our code from Compiler 2 
and implement methods that would allow it add in compatibility for Functions, AND, OR, 
and NOT operations. 
'''
import sys, string
...
def block(tableIndex, level):
    ...
    while sym == "PROCEDURE" or sym == "VAR" or sym == "CONST": 
        ...
        while sym == "PROCEDURE" or sym == "FUNCTION": 
            ###ADD or sym == "FUNCTION": in FUNCTION
            PFsym = sym 
            getsym()
            if sym == "ident":
                ###IF SYM is procedure, enter procedure
                if PFsym == "PROCEDURE": 
                    enter(tx, 'procedure', level, codeIndx)
                ###IF SYM is function, enter function 
                ###Add in 'function'
                if PFsym == "FUNCTION": 
...
def statement(tx, level):
    ...
    if sym == "ident":
        ...
        if i==0:
            error(11)
        elif table[i].kind != "variable" or table[i].kind != "function":
            error(12)
        getsym()
        if sym != "becomes":
            error(13)
        ...
        if kind == "function":  ###Add in its body 
            gen("STO", 0, -1)
...
def expression(tx, level):
    global sym;
    if sym == "plus" or sym == "minus" or sym == ": 
        addop = sym
        getsym()
        term(tx, level)
...
def term(tx, level):
    ...
    while sym=="times" or sym=="slash" or sym == "AND":
        ...
        if mulop == "slash": 
            ...
            gen("OPR", 0, 4)        #multiply operation2
...
def factor(tx, level):
    ...
    if sym == "ident":
        ...
        ###Add in the var, constant, and procedure/function
        if sym == "variable": 
            gen ("LOD", level - table[i].level, table[i].adr)
        if sym == "const":
            gen ("LIT", 0, table[i].value)
        if sym == "procedure" or sym == "function": 
            error(39)
