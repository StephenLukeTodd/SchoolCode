###Credit for Problem comes from: https://www.hackerearth.com/practice/algorithms/graphs/graph-representation/practice-problems/algorithm/monk-in-the-real-estate/
T_Legal = False
while not T_Legal:
	print('\n')
...
for i in range(T):
	...
	for j in range(E):
		...
		while not CityX_Legal or not CityY_Legal:
			...
			#CityX = int(input())
			#CityY = int(input())
			CityX, CityY = [int(x) for x in raw_input("Enter two Numbers:").split()]
